#!/usr/bin/env bash
# Optimized build script with caching and incremental builds
set -eu
...
# Check for incremental build flag
...
if [ "${INCREMENTAL}" == "1" ] && [ -d "${DIR}/work/stage2" ]; then
	echo "🚀 Incremental build mode enabled"
	if [ -f "${DIR}/work/stage2/export-image/"*.img ] || [ -f "${DIR}/work/stage2/EXPORT_IMAGE" ]; then
		echo "✅ Found stage2 artifacts - will skip stages 0-2"
		SKIP_STAGES="stage0 stage1 stage2"
		# Create SKIP files
		mkdir -p "${DIR}/work/stage0" "${DIR}/work/stage1" "${DIR}/work/stage2"
		touch "${DIR}/work/stage0/SKIP" "${DIR}/work/stage1/SKIP" "${DIR}/work/stage2/SKIP"
		# Remove stage3 SKIP if exists
		rm -f "${DIR}/work/stage3/SKIP"
	else
		echo "⚠️  No stage2 artifacts found - will run full build"
		INCREMENTAL=0
	fi
fi
...
if [ "${INCREMENTAL}" == "1" ]; then
	echo "📦 Running INCREMENTAL build (stage3 only)"
else
	echo "📦 Running FULL build (all stages)"
fi
...
time ${DOCKER} run \
  ...
  bash -e -o pipefail -c "
    ...
    cd /pi-gen; 
    # Apply skip stages if incremental
    if [ -n \"${SKIP_STAGES}\" ]; then
      for stage in ${SKIP_STAGES}; do
        echo \"Skipping \${stage}\"
        mkdir -p work/\${stage}
        touch work/\${stage}/SKIP
      done
    fi
    ./build.sh ${BUILD_OPTS} &&
